function [A,N] = PoissonInitRobin(nx,ny,hx,hy,alpha,ii,jj,px,py)
%
...
% Prise en compte des conditions de Robin 
    ...
    for i=1: nx+1
        ...
        A(k,:)=0;A(k,k)=1.+alpha*hy;A(k,km)=-1.;
        else
            A(k,:)=0;A(k,k)=1;
        end
        ...
        kp= i + (nx+1);
       if(jj>1)
        A(k,:)=0;A(k,k)=1+alpha*hy;A(k,kp)=-1.;
       else
           A(k,:)=0;A(k,k)=1;
        end
   ...
   for j=1: ny+1
        ...
        kg= nx + (j-1)*(nx+1);  
        if(ii<px)
        A(k,:)=0;A(k,k)=1.+alpha*hx;A(k,kg)=-1.;
        else
            A(k,:)=0;A(k,k)=1;
        end
        ...
        kd= 2 + (j-1)*(nx+1);  
        if(ii>1)
        A(k,:)=0;A(k,k)=1+alpha*hx;A(k,kd)=-1.;
        else
            A(k,:)=0;A(k,k)=1;
        end
